...
"px","box-shadow":"0 0 4px black"}).removeClass("fullscreen");else{if($(this.root).hasClass("fullscreen"))return;$(this.root).addClass("fullscreen");$(this.root).css({width:"100%",height:"100%","box-shadow":"0 0 0"})}$(LiteGUI).trigger("resized")},maximizeWindow:function(){this.setWindowSize()},setCursor:function(c){this.root.style.cursor=c},isCursorOverElement:function(c,b){var a=c.pageX,e=c.pageY,d=b.getClientRects()[0];return d?e>d.top&&e<d.top+d.height&&a>d.left&&a<d.left+d.width?!0:!1:!1},getRect:function(c){return c.getClientRects()[0]},
toClipboard:function(c,b){c&&c.constructor!==String&&(c=JSON.stringify(c));var a=null,e=!1;if(!b)try{document.queryCommandSupported("copy"),a=document.createElement("input"),a.type="text",a.style.opacity=0,a.value=c,document.body.appendChild(a),a.select(),e=document.execCommand("copy"),console.log(e?"saved to clipboard":"problem saving to clipboard"),document.body.removeChild(a)}catch(d){a&&document.body.removeChild(a),console.warn("Oops, unable to copy using the true clipboard")}localStorage.setItem("litegui_clipboard",
...
f-1,0,2,k.height);a!=this.value&&(this.value=a,c||LiteGUI.trigger(this.root,"change",a))};k.addEventListener("mousedown",function(a){var b;a.offsetX?b=a.offsetX:a.layerX&&(b=a.layerX);c(b);document.addEventListener("mousemove",g);document.addEventListener("mouseup",h)});this.setValue(a)};LiteGUI.LineEditor=function(a,b){function c(a){return[q.width*((m.xrange[1]-m.xrange[0])*a[0]+m.xrange[0]),q.height*((m.yrange[1]-m.yrange[0])*a[1]+m.yrange[0])]}function g(a){return[(a[0]/q.width-m.xrange[0])/(m.xrange[1]-
m.xrange[0]),(a[1]/q.height-m.yrange[0])/(m.yrange[1]-m.yrange[0])]}function h(a){var b=q.getBoundingClientRect(),c=a.clientX-b.left,e=a.clientY-b.top;0>c?c=0:c>q.width&&(c=q.width);0>e?e=0:e>q.height&&(e=q.height);if(-1!=r&&n([a.clientX-b.left,a.clientY-b.top],[c,e])>0.5*q.height)m.value.splice(r,1),k(a);else{b=g([-(s[0]-c),s[1]-e]);if(-1!=r){var d=m.xrange[0],f=m.xrange[1];m.no_trespassing&&(0<r&&(d=m.value[r-1][0]),r<m.value.length-1&&(f=m.value[r+1][0]));var h=m.value[r];h[0]+=b[0];h[1]+=b[1];
h[0]<d?h[0]=d:h[0]>f&&(h[0]=f);h[1]<m.yrange[0]?h[1]=m.yrange[0]:h[1]>m.yrange[1]&&(h[1]=m.yrange[1])}p();m.redraw();s[0]=c;s[1]=e;l();a.preventDefault();a.stopPropagation()}}function k(a){r=-1;m.redraw();$(document).unbind("mousemove",h);$(document).unbind("mouseup",k);l();a.preventDefault();a.stopPropagation()}function l(){b.callback?b.callback.call(m,m.value):$(m).change()}function n(a,b){return Math.sqrt(Math.pow(b[0]-a[0],2)+Math.pow(b[1]-a[1],2))}function p(){var a=null;-1!=r&&(a=m.value[r]);
...
!1;f.hidePanels()},LiteGUI.Menubar.closing_time)});g.addEventListener("mouseenter",function(a){f.closing_by_leave&&clearInterval(f.closing_by_leave);f.closing_by_leave=null});c=$(c);g.style.left=c.offset().left+(d?200:0)+"px";g.style.top=c.offset().top+c.height()+(d?-20:10)+"px";this.panels.push(g);document.body.appendChild(g)}};LiteGUI.Menubar=c})();
...
jQuery.fn.wchange=function(c){$(this[0]).on("wchange",c)};jQuery.fn.wclick=function(c){$(this[0]).on("wclick",c)};
...
b&&(this.value=b)});d.querySelector(".wcontent button").addEventListener("click",function(b){a.callback_button&&a.callback_button.call(d,f.value,b)});this.tab_index+=1;this.append(d,a);d.wchange=function(a){$(this).wchange(a)};d.wclick=function(a){$(this).wclick(a)};d.setValue=function(a,b){f.value=a;b||LiteGUI.trigger(f,"change")};d.getValue=function(){return f.value};d.focus=function(){LiteGUI.focus(f)};this.processElement(d,a);return d};
...
Inspector.prototype.addNumber=function(c,b,a){a=this.processOptions(a);a.step||(a.step=0.1);b=b||0;var e=this;this.values[c]=b;var d=this.createWidget(c,"",a);this.append(d,a);a.extraclass="full";a.tab_index=this.tab_index;a.full=!0;this.tab_index++;var f=null,f=new LiteGUI.Dragger(b,a);f.root.style.width="calc( 100% - 1px )";d.querySelector(".wcontent").appendChild(f.root);f.root.addEventListener("start_dragging",function(a){this.callback_before&&this.callback_before.call(d)}.bind(a));a.disabled&&
f.input.setAttribute("disabled","disabled");var g=d.querySelector("input");g.addEventListener("change",function(b){LiteGUI.trigger(d,"wbeforechange",b.target.value);e.values[c]=b.target.value;if(a.callback){var f=a.callback.call(d,parseFloat(b.target.value));"number"==typeof f&&(this.value=f)}LiteGUI.trigger(d,"wchange",b.target.value);if(e.onchange)e.onchange(c,b.target.value,d)});d.setValue=function(b,c){void 0!==b&&(b=parseFloat(b),a.precision&&(b=b.toFixed(a.step&&a.step<a.precision?a.step:a.precision)),
b+=a.units||"",g.value!=b&&(g.value=b,c||LiteGUI.trigger(g,"change")))};d.getValue=function(){return parseFloat(g.value)};d.focus=function(){LiteGUI.focus(g)};this.processElement(d,a);return d};
...
a);k.root.style.width="calc( 50% - 1px )";g.appendChild(k.root);$(h.root).bind("start_dragging",e.bind(a));$(k.root).bind("start_dragging",e.bind(a));$(f).find("input").change(function(b){b=[];for(var e=$(f).find("input"),g=0;g<e.length;g++)b.push(parseFloat(e[g].value));LiteGUI.trigger(f,"wbeforechange",[b]);d.values[c]=b;if(a.callback){var h=a.callback.call(f,b);if("object"==typeof h&&2<=h.length){for(g=0;g<e.length;g++)$(e[g]).val(h[g]);b=h}}LiteGUI.trigger(f,"wchange",[b]);if(d.onchange)d.onchange(c,
b,f)});this.append(f,a);f.setValue=function(a,b){a&&(h.getValue()!=a[0]&&h.setValue(a[0],!0),k.getValue()!=a[1]&&k.setValue(a[1],b))};this.processElement(f,a);return f};
...
a);h.root.style.width="calc( 33% - 1px )";f.querySelector(".wcontent").appendChild(h.root);a.tab_index=this.tab_index;this.tab_index++;var k=new LiteGUI.Dragger(b[2],a);k.root.style.width="calc( 33% - 1px )";f.querySelector(".wcontent").appendChild(k.root);g.root.addEventListener("start_dragging",e.bind(a));h.root.addEventListener("start_dragging",e.bind(a));k.root.addEventListener("start_dragging",e.bind(a));$(f).find("input").change(function(b){b=[];for(var e=$(f).find("input"),g=0;g<e.length;g++)b.push(parseFloat(e[g].value));
LiteGUI.trigger(f,"wbeforechange",[b]);d.values[c]=b;if(a.callback){var h=a.callback.call(f,b);if("object"==typeof h&&3<=h.length){for(g=0;g<e.length;g++)$(e[g]).val(h[g]);b=h}}LiteGUI.trigger(f,"wchange",[b]);if(d.onchange)d.onchange(c,b,f)});this.append(f,a);f.setValue=function(a,b){a&&(g.setValue(a[0],!0),h.setValue(a[1],!0),k.setValue(a[2],b))};this.processElement(f,a);return f};
...
k.root.addEventListener("start_dragging",e.bind(a));g.push(k)}$(f).find("input").change(function(b){b=[];for(var e=$(f).find("input"),g=0;g<e.length;g++)b.push(parseFloat(e[g].value));LiteGUI.trigger(f,"wbeforechange",[b]);d.values[c]=b;if(a.callback){var h=a.callback.call(f,b);if("object"==typeof h&&4<=h.length){for(g=0;g<e.length;g++)$(e[g]).val(h[g]);b=h}}LiteGUI.trigger(f,"wchange",[b]);if(d.onchange)d.onchange(c,b,f)});this.append(f,a);f.setValue=function(a,b){if(a)for(var c=0;c<g.length;c++)g[c].setValue(a[c],
b)};this.processElement(f,a);return f};
...
Inspector.prototype.addComboButtons=function(c,b,a){a=this.processOptions(a);b=b||"";var e=this;this.values[c]=b;var d="";if(a.values)for(var f in a.values)d+="<button class='wcombobutton "+(b==a.values[f]?"selected":"")+"' data-name='options.values[i]'>"+a.values[f]+"</button>";var g=this.createWidget(c,d,a);b=g.querySelectorAll(".wcontent button");LiteGUI.bind(b,"click",function(b){b=b.target.innerHTML;e.values[c]=b;$(g).find(".selected").removeClass("selected");this.classList.add("selected");Inspector.onWidgetChange.call(e,
g,c,b,a)});this.append(g,a);this.processElement(g,a);return g};
Inspector.prototype.addTags=function(c,b,a){function e(b){if(!g.tags[b]){LiteGUI.trigger(g,"wbeforechange",g.tags);g.tags[b]=!0;var e=document.createElement("div");e.data=b;e.className="wtag";e.innerHTML=b+"<span class='close'>X</span>";e.querySelector(".close").addEventListener("click",function(b){b=$(this).parent()[0].data;delete g.tags[b];LiteGUI.remove(this.parentNode);$(g).trigger("wremoved",b);Inspector.onWidgetChange.call(d,g,c,g.tags,a)});g.querySelector(".wtagscontainer").appendChild(e);
d.values[c]=g.tags;a.callback&&a.callback.call(g,g.tags);LiteGUI.trigger(g,"wchange",g.tags);LiteGUI.trigger(g,"wadded",b);if(d.onchange)d.onchange(c,g.tags,g)}}a=this.processOptions(a);b=b||[];var d=this;this.values[c]=b;b="<select>";if(a.values)for(var f in a.values)b+="<option>"+a.values[f]+"</option>";var g=this.createWidget(c,"<span class='inputfield full'>"+(b+"</select><div class='wtagscontainer inputfield'></div>")+"</span>",a);g.tags={};for(f in a.value)e(a.value[f]);$(g).find(".wcontent select").change(function(a){e(a.target.value)});
this.append(g,a);this.processElement(g,a);return g};
Inspector.prototype.addList=function(c,b,a){function e(a){var b=$(k).find("li.selected");b&&b.length&&(40==a.keyCode?(a=b.next())&&a.length&&$(a[0]).click():38==a.keyCode&&(a=b.prev())&&a.length&&$(a[0]).click())}function d(d){a.multiselection?this.classList.toggle("selected"):($(k).find("li").removeClass("selected"),this.classList.add("selected"));d=b[this.dataset.pos];Inspector.onWidgetChange.call(g,k,c,d,a);$(k).trigger("wadded",d)}function f(c){c=b[this.dataset.pos];a.callback_dblclick&&a.callback_dblclick.call(g,
c)}a=this.processOptions(a);var g=this,h="";a.height&&(h="style='height: "+a.height+"px; overflow: auto;'");h="<ul class='lite-list' "+h+" tabIndex='"+this.tab_index+"'><ul>";this.tab_index++;var k=this.createWidget(c,"<span class='inputfield full "+(a.disabled?"disabled":"")+"'>"+h+"</span>",a);$(k).find("ul").focus(function(){$(document).on("keypress",e)});$(k).find("ul").blur(function(){$(document).off("keypress",e)});k.updateItems=function(c,e){e=e||a.selected;b=c;var g=this.querySelector("ul");
g.innerHTML="";if(b)for(var h in b){var k=b[h],r=b.constructor===Array?k:h;r||(r=h);var s=r.constructor===String?r:h,t=null;r&&r.constructor===String&&(r=r.replace(/<(?:.|\n)*?>/gm,""));var y="";null!==k&&void 0!==k&&k.constructor!==String&&k.constructor!==Number&&k.constructor!==Boolean&&k&&(s=k.content||k.name||h,t=k.style,k.icon&&(y="<img src='"+k.icon+"' class='icon' />"));k=!1;if("object"==typeof b[h]&&b[h].selected||e==b[h])k=!0;var u=document.createElement("li");u.classList.add("item-"+LiteGUI.safeName(h));
k&&u.classList.add("selected");u.dataset.name=r;u.dataset.pos=h;t&&u.setAttribute("style",t);u.innerHTML=y+s;g.appendChild(u);u.addEventListener("click",d);a.callback_dblclick&&u.addEventListener("dblclick",f)}LiteGUI.bind(g.querySelectorAll("li"),"click",d)};k.removeItem=function(a){for(var b=k.querySelectorAll(".wcontent li"),c=0;c<b.length;c++)b[c].dataset.name==a&&LiteGUI.remove(b[c])};k.updateItems(b,a.selected);this.append(k,a);k.getSelected=function(){for(var a=[],b=this.querySelectorAll("ul li.selected"),
c=0;c<b.length;++c)a.push(b[c].dataset.name);return a};k.getIndex=function(a){return this.querySelectorAll("ul li")[a]};k.selectIndex=function(a,b){for(var c=this.querySelectorAll("ul li"),d=0;d<c.length;++d){var e=c[d];d==a?e.classList.add("selected"):b||e.classList.remove("selected")}return c[a]};k.scrollToIndex=function(a){if(a=this.querySelectorAll("ul li")[a])this.scrollTop=a.offsetTop};k.selectAll=function(){for(var a=this.querySelectorAll("ul li"),b=0;b<a.length;++b){var c=a[b];c.classList.contains("selected")||
LiteGUI.trigger(c,"click")}};k.setValue=function(a){void 0!==a&&this.updateItems(a)};a.height&&$(k).scroll(0);this.processElement(k,a);return k};
...
a);g.wclick=function(b){a.disabled||$(this).wclick(b)};g.setValue=function(a){void 0!==a&&(h.innerHTML=a)};this.processElement(g,a);return g};
...
Inspector.prototype.addColor=function(c,b,a){a=this.processOptions(a);b=b||[0,0,0];var e=this;this.values[c]=b;var d="<input tabIndex='"+this.tab_index+"' id='colorpicker-"+c+"' class='color' value='"+(b[0]+","+b[1]+","+b[2])+"' "+(a.disabled?"disabled":"")+"/>";this.tab_index++;a.show_rgb&&(d+="<span class='rgb-color'>"+Inspector.parseColor(b)+"</span>");var f=this.createWidget(c,d,a);this.append(f,a);var d=f.querySelector("input.color"),g=null,g=new jscolor.color(d);g.pickerFaceColor="#333";g.pickerBorderColor=
"black";g.pickerInsetColor="#222";g.rgb_intensity=1;a.disabled&&(g.pickerOnfocus=!1);b.constructor!==String&&b.length&&2<b.length&&(g.fromRGB(1*b[0],1*b[1],1*b[2]),g.rgb_intensity=1);d.addEventListener("change",function(a){if(a=f.querySelector(".rgb-color"))a.innerHTML=LiteGUI.Inspector.parseColor(g.rgb)});g.onImmediateChange=function(){var b=[g.rgb[0]*g.rgb_intensity,g.rgb[1]*g.rgb_intensity,g.rgb[2]*g.rgb_intensity],d=[b.concat(),g.toString()];LiteGUI.trigger(f,"wbeforechange",d);e.values[c]=b;
a.callback&&a.callback.call(f,b.concat(),"#"+g.toString(),g);LiteGUI.trigger(f,"wchange",d);if(e.onchange)e.onchange(c,b.concat(),f)};a.step=a.step||0.01;a.dragger_class="nano";var h=new LiteGUI.Dragger(1,a);f.querySelector(".wcontent").appendChild(h.root);h.input.addEventListener("change",function(a){a=parseFloat(this.value);g.rgb_intensity=a;if(g.onImmediateChange)g.onImmediateChange()});f.setValue=function(a,b){g.fromRGB(a[0],a[1],a[2]);b||LiteGUI.trigger(h.input,"change")};this.processElement(f,
a);return f};
Inspector.prototype.addFile=function(c,b,a){a=this.processOptions(a);b=b||"";var e=this;this.values[c]=b;var d=this.createWidget(c,"<span class='inputfield full whidden' style='width: calc(100% - 26px)'><span class='filename'>"+b+"</span></span><button class='litebutton' style='width:20px; margin-left: 2px;'>...</button><input type='file' size='100' class='file' value='"+b+"'/>",a);d.querySelector(".wcontent").style.position="relative";b=d.querySelector(".wcontent input");var f=d.querySelector(".wcontent .filename");b.addEventListener("change",
function(b){if(b.target.files.length){var h=null;a.generate_url&&(h=URL.createObjectURL(b.target.files[0]));h={url:h,filename:b.target.value,file:b.target.files[0],files:b.target.files};f.innerHTML=b.target.files[0].name;Inspector.onWidgetChange.call(e,d,c,h,a)}else f.innerHTML="",Inspector.onWidgetChange.call(e,d,c,null,a)});this.append(d,a);return d};
Inspector.prototype.addLine=function(c,b,a){a=this.processOptions(a);b=b||"";var e=this;this.values[c]=b;var d=this.createWidget(c,"<span class='line-editor'></span>",a);b=new LiteGUI.LineEditor(b,a);$(d).find("span.line-editor").append(b);$(b).change(function(b){LiteGUI.trigger(d,"wbeforechange",[b.target.value]);a.callback&&a.callback.call(d,b.target.value);LiteGUI.trigger(d,"wchange",[b.target.value]);Inspector.onWidgetChange.call(e,d,c,b.target.value,a)});this.append(d,a);return d};
Inspector.prototype.addTree=function(c,b,a){a=this.processOptions(a);b=b||"";var e=this.createWidget(c,"<div class='wtree inputfield full'></div>",a);c=$(e).find(".wtree")[0];a.height&&(c.style.height="number"==typeof a.height?a.height+"px":a.height,c.style.overflow="auto");var d=e.tree=new LiteGUI.Tree(null,b,a.tree_options);d.onItemSelected=function(b,c){a.callback&&a.callback.call(e,b,c)};c.appendChild(d.root);e.setValue=function(a){d.updateTree(a)};this.append(e,a);this.processElement(e,a);return e};
Inspector.prototype.addDataTree=function(c,b,a){function e(a,b){for(var c in b){var d=document.createElement("div");d.className="treenode";"object"==typeof b[c]?(d.innerHTML="<span class='itemname'>"+c+"</span><span class='itemcontent'></span>",e($(d).find(".itemcontent")[0],b[c])):d.innerHTML="<span class='itemname'>"+c+"</span><span class='itemvalue'>"+b[c]+"</span>";a.appendChild(d)}}a=this.processOptions(a);b=b||"";c=this.createWidget(c,"<div class='wtree'></div>",a);var d=c.querySelector(".wtree");
e(d,b);this.append(c,a);return c};
Inspector.prototype.addArray=function(c,b,a){function e(){var b=this.value,c=h.length,e=b.length;if(e<c){for(var n=e;n<h.length;++n)h[n]&&h[n].remove();h.length=e}if(e>c)for(n=c;n<e&&n<l;++n){c=null;void 0!==b[n]&&(c=b[n]);var s={widget_parent:g,name_width:30,callback:d.bind({value:this.value,index:n})};if(a.data_options)for(var t in a.data_options)s[t]=a.data_options[t];c=f.add(k,n,c,s);h.push(c)}}function d(b){this.value[this.index]=b;a.callback&&a.callback.call(g,this.value,this.index)}var f=this;
